...
//	CWLog("[F:%s, L:%d]",__FILE__,__LINE__);
	//CW_FREE_OBJECT(gWTPs[selection].vendorValues);
	...
	//CW_FREE_OBJECT(gWTPs[selection].vendorValues->payload);
...
//				CWLog("[F:%s, L:%d]  gWTPs[i].WTPProtocolManager.name:%s",__FILE__,__LINE__,gWTPs[i].WTPProtocolManager.name);
				CWLog("[F:%s, L:%d]  gActiveWTPsTemp:%d",__FILE__,__LINE__,gActiveWTPsTemp);
			...
			if ( cmd_msg == CONF_UPDATE_MSG ) {
				...
				CWLog("CONF_UPDATE_MSG msg_elem = %d",msg_elem);
				wtpIndex = ntohl(wtpIndex);
				CWLog("CONF_UPDATE_MSG wtpIndex = %d",wtpIndex);
				...
				case MSG_ELEMENT_TYPE_VENDOR_XML:
				/* Antonio Case */
				CW_CREATE_OBJECT_ERR(xmlValues, CWVendorXMLValues, {CWErrorRaise(CW_ERROR_OUT_OF_MEMORY, NULL); return 0;});
				if ( (n = Readn(sock, commandBuffer, sizeof(char)+sizeof(int))) < 0) {
					CWLog("Error while reading from socket");
					goto quit_manage;
				}

				memcpy(&xmlValues->wum_type, commandBuffer, sizeof(unsigned char)); /* CONF_UPDATE type */
				memcpy(&xmlValues->payloadLen, commandBuffer + sizeof(unsigned char), sizeof(int));	/* WTP Index */
				CWLog("xmlValues->wum_type = %d",xmlValues->wum_type);
				xmlValues->payloadLen = ntohl(xmlValues->payloadLen);

				if(!xmlValues->payloadLen)
				{
				...
				CWLog("xmlValues->payloadLen = %d",xmlValues->payloadLen);
				memset(xmlValues->payload,0,xmlValues->payloadLen);
				n = 0;
				while( n!=xmlValues->payloadLen)
				{
					if ( (n += Readn(sock, commandBuffer+n, xmlValues->payloadLen))< 0) {
									CWLog("Error while reading from socket");
									goto quit_manage;
								}
				...
				if(numActiveWTPs>0) {
					for(i=0; i<CW_MAX_WTP; i++) {
						if(gWTPs[i].isNotFree)  {
							CWXMLSetValues(i, socketIndex, xmlValues);
						}
					}
				}
			}
			else /* One specific Wtp Case */
				CWXMLSetValues(wtpIndex, socketIndex, xmlValues);
//			CWLog("[F:%s, L:%d]",__FILE__,__LINE__);
