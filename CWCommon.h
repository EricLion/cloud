...
#define	CW_FREE_OBJECT(obj_name)		{if((obj_name) != NULL){free((obj_name)); (obj_name) = NULL;}}
...
#define	CW_CREATE_STRING_ERR(str_name, str_length, on_err)	{(str_name) = (char*) (malloc(sizeof(char) * ((str_length)) ) ); if(!(str_name)) {on_err}}
